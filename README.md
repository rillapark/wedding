...
사이트는 누구나 볼 수 있지만, **저장하려면 편집 암호가 필요**합니다. 링크만 아는 사람이 배치를 지우지 못하게 하기 위한 것입니다.

상단 ‘편집 암호’ 버튼으로 입력하면 그 브라우저에 기억됩니다. 암호는 Vercel 프로젝트의 `EDIT_KEY` 환경 변수이며, Vercel 대시보드 → Settings → Environment Variables 에서 바꿀 수 있습니다. 바꾼 뒤에는 재배포해야 적용됩니다.

암호 없이 누구나 편집하게 하려면 `EDIT_KEY` 변수를 삭제하고 재배포하세요.

### API

| 메서드 | 경로 | 설명 |
|---|---|---|
| `GET` | `/api/plan` | 현재 배치와 `etag`를 반환합니다. 저장된 배치가 없으면 `plan: null`. |
| `PUT` | `/api/plan` | `{ plan, etag }`를 저장합니다. 헤더 `x-edit-key` 필요. |
