...
- 공유 배치는 **어떤 명단 파일로 만들어졌는지**(`rosterHash`)를 함께 기록합니다. `dist/data/guests.xlsx`를 교체하고 배포하면, 다음 방문 때 새 명단을 자동으로 반영하고 기존 좌석은 **이름+구분이 같은 하객만** 그대로 유지합니다. 새 명단에 없는 하객의 좌석은 해제되고 그 사실을 알려 줍니다.
